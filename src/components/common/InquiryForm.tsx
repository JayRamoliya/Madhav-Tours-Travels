...
import { Send, MessageSquare, Phone, Calendar, Users, MapPin, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const InquiryForm: React.FC<InquiryFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Save inquiry to session storage so Thank You page can display personalized confirmation
    try {
    ...
    // Prepare pre-filled WhatsApp message as requested:
    // "Hi Madhav Tours & Travels, I'm interested in a travel package. Please share details and quotation."
    const customWhatsAppText = `Hi Madhav Tours & Travels, I'm interested in a travel package. Please share details and quotation.
Name: ${formData.name}
Phone: ${formData.phone}
Destination / Service: ${formData.destination || formData.serviceType || 'Custom Tour'}
Date: ${formData.travelDate || 'Flexible'}
Travelers: ${formData.travelers}
${formData.message ? `Notes: ${formData.message}` : ''}`;

    const waUrl = createWhatsAppLink(customWhatsAppText);
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setSameAsPhone(next);
                    if (next) setFormData((prev) => ({ ...prev, whatsapp: prev.phone }));
                  }}
          ...
          <div className="flex flex-wrap items-center justify-between text-xs text-slate-500 py-1 border-t border-slate-100 gap-2">
            <span className="flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-[#00A86B]" /> 100% Free Consultation
            </span>
            <span className="flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-[#00A86B]" /> No Spam / No Bot Calls
            </span>
            <span className="flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-[#00A86B]" /> Direct WhatsApp Quote
            </span>
          </div>
          ...
          <div className="pt-2 flex flex-col sm:flex-row gap-3">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <span>Send Inquiry & Get WhatsApp Quote</span>
